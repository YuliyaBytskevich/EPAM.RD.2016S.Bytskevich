namespace UserStorage
{
    ...
    public static class ApplicationManager
    {
        ...
        public static void ConfigureAppServces()
        {
            ...
            if (section != null)
            {
                ...
                for (int i = 0; i < itemsCount; i++)
                {
                    ...
                    {
                        throw new ApplicationException("Application manager doesn't support service type '" +
                                                       section.SectionItems[i].ServiceType + "'.");
                    }
                }
        ...
        private static UserStorageService CreateService(ServiceElement config)
        {
            ...
            PrimeNumbersGenerator generator = (PrimeNumbersGenerator)domain.CreateInstanceAndUnwrap(
                                                                     "UserStorage", 
                                                                     "UserStorage.IdentifiersGeneration.PrimeNumbersGenerator",
                                                                      false, 
                                                                      ...
                                                                      null, 
                                                                      null);
            InMemoryUserStorage storage = (InMemoryUserStorage)domain.CreateInstanceAndUnwrap(
                                                               "UserStorage", 
                                                               "UserStorage.Repositories.InMemoryUserStorage",
                                                               false, 
                                                               BindingFlags.CreateInstance | BindingFlags.Public | BindingFlags.Instance,
                                                               null, 
            ...
            {
                service = (MasterService)domain.CreateInstanceAndUnwrap(
                                         "UserStorage", 
                                         "UserStorage.Services.MasterService",
                                         false, 
                                         BindingFlags.CreateInstance | BindingFlags.Public | BindingFlags.Instance,
                                         null, 
                                         new object[] { config.ServiceIdentifier, config.XmlPath, storage }, 
                                         null, 
                                         null);
            }
            else
            {
                service = (SlaveService)domain.CreateInstanceAndUnwrap(
                                        "UserStorage", 
                                        "UserStorage.Services.SlaveService",
                                        false, 
                                        BindingFlags.CreateInstance | BindingFlags.Public | BindingFlags.Instance,
                                        null, 
                                        new object[] { config.ServiceIdentifier, config.XmlPath, config.Port, storage }, 
                                        null, 
                                        null);
            }
